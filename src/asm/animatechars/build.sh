...
UPPERCASE_BASENAME=$(echo "${BASENAME}" | awk '{ print toupper($0) }')

writeToDiskCommand="/usr/local/bin/ac -p ${DEST_DSK} ${UPPERCASE_BASENAME} bin 0x0300 < ./src/asm/${BASENAME}/${BASENAME}"
